...
def perform_linear_regression(dataframe, training_set_fraction=0.7):
    ...
    x = dataframe.drop(columns=["CoworkingSpaces"])
    y = dataframe["CoworkingSpaces"]
    ...
    plt.legend()
    plt.title("Linear Regression Model Predicting CoworkingSpaces")
    plt.show()


def perform_linear_regression1(dataframe, training_set_fraction=0.7):
    """
    Perform linear regression on the dataset.
    :param dataframe: Pandas DataFrame with features and target variable.
    :param training_set_fraction: Fraction of data to use for training.
    :return: Plot with regression line and y=x line.
    """
    ...
    x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=1 - training_set_fraction, random_state=1)
    ...
    dataframe_with_pred = pd.concat([x_test_df, y_test.reset_index(drop=True)], axis=1)

    # Test with simplified formula
    try:
        formula = 'Ranking ~ ' + '+'.join(x.columns)
        smf_model = smf.ols(formula=formula, data=dataframe_with_pred)
        results = smf_model.fit()
        y_predicted = results.predict(x_test_df)

        # Show the summary of the statsmodels linear regression
        print(results.summary())

        # Plot
        plt.scatter(y_test, y_predicted, label='Predicted vs Actual')
        ...
        # Adding a legend, title, and showing the plot
        plt.legend()
        plt.title("Linear Regression Model Predicting Ranking")
        plt.show()
...
def initiate_log_regression(train_data, test_data):
    ...
    y_train = train_data['CoworkingSpaces']
    x_train = train_data.drop(columns=['CoworkingSpaces'])
    y_test = test_data['CoworkingSpaces']
    x_test = test_data.drop(columns=["CoworkingSpaces"])
...
def create_xgboost_classifier(X_train, y_train, X_test, y_test):
    ...
    print(classification_report(y_test, y_predict))
    # print('Random Forest Score: {0:2.2f}'.format(xgb.score(X_test, y_test)))
    #
    # print('Scores on Training Set:')
    # print('Random Forest Score: {0:2.2f}'.format(xgb.score(X_train, y_train)))
    # print('Scores on Test Set:')
    # print('Random Forest Score: {0:2.2f}'.format(xgb.score(X_test, y_test)))
    return y_predict
...
def main():
    ...
    # phase 4 - logistics regression
    # print("\nLogistic Regression!:\n")
    # binary_tree_df = turn_col_to_binary(encoded_df, 'Ranking')
    # log_data_train, log_data_test = split_log_regression(binary_tree_df, 0.7)
    # initiate_log_regression(log_data_train, log_data_test)

    # # phase 3 - Model Training and Evaluation
    # df_without_target = encoded_df.drop(columns=['RecordNumber', 'CustomerId', 'Retention', 'LastName'])
    # X_train, X_test, y_train, y_test = train_test_split(df_without_target, encoded_df['Retention'], test_size=0.3,
    #                                                     random_state=42)
    ...
    # create_xgboost_classifier(X_train, y_train, X_test, y_test)
